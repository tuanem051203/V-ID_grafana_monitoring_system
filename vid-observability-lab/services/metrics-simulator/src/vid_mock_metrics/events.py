...
class EventScheduler:
    ...
    def active_at(self, second_of_day: float) -> EventEffects:
        ...
        if not active:
            return EventEffects()
        http_rates = [event.http_5xx_rate for event, _ in active if event.http_5xx_rate is not None]
        database_rates = [
        ...
        return EventEffects(
            ...
            traffic_multiplier=math_product(
                1 + (event.traffic_multiplier - 1) * weight for event, weight in active
            ),
            auth_success_penalty=sum(
                event.auth_success_penalty * weight for event, weight in active
            ),
            auth_latency_multiplier=math_product(
                1 + (event.auth_latency_multiplier - 1) * weight for event, weight in active
            ),
            ...
            token_latency_multiplier=math_product(
                1 + (event.token_latency_multiplier - 1) * weight for event, weight in active
            ),
            http_5xx_rate=max(http_rates) if http_rates else None,
            database_latency_multiplier=math_product(
                1 + (event.database_latency_multiplier - 1) * weight for event, weight in active
            ),
